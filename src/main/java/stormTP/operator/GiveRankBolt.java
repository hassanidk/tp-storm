...
public class GiveRankBolt implements IRichBolt {
	...
	public String getRank(int devant, int derriere, int total){
		String position;
		int calculExeco;
		
		position = String.valueOf(total - derriere);
		calculExeco = devant + derriere + 1;
		if (calculExeco != total) {
	...
	public void execute(Tuple t) {
		...
		try {
		    ...
		    r.put("rank", getRank(nbDevant, nbDerriere, total));
		    r.put("total", total);
